...
class bot extends Modul{
    public $host="https://nfiles.xyz/feyorra/";
    public $asu ="https://nfiles.xyz/feyorra/xplay_v4.php?mail=";
    public $path="78484_daw48.php?id=";
    public function __construct(){
        $this->ua=[
            "Host: nfiles.xyz",
            "user-agent: ".self::uagent(),
            "accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "x-requested-with: free.feyorra",
            "sec-fetch-site: none",
        ...
        $bot="feyorraApp";$ver="1.0";$con="data.json";
        ...
        $aw=$this->asd($data["email"]);self::timer("sabar menunggu ya",60);
        preg_match('#bckk= "(.*?)"#i',$aw,$link);
        if($link[1] != null){
            $url=self::curl($link[1],'',$this->ua($data["email"]))[1];
            preg_match("#('(.*?)')#is",$url,$hsl);
            echo self::col("suksess claim ","h").self::col($hsl[2],"c").self::col(" feyorra","b")."\n";self::strip();
            continue;
        }
        endwhile;
    }
    public function hos($mail){
        ...
        return self::curl($this->host.'feyya.php',$data,$this->ua($this->asu))[1];
    }
    public function ua($da){
        return array_merge($this->ua,["referer: ".$this->asu.$da]);
    }
    public function asd($mail){
        return self::curl($this->host.'xplay.php?mail='.$mail,'',$this->ua($this->asu))[1];
    }
}
(new bot);
